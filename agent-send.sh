...
get_agent_target() {
    case "$1" in
        "president") echo "president-akasatanamidakuji" ;;
        "boss1"|"worker1"|"worker2"|"worker3")
            ...
            if tmux has-session -t "multiagent-akasatanamidakuji" 2>/dev/null; then
                local indices=($(get_tmux_indices "multiagent-akasatanamidakuji"))
                ...
                # pane番号を計算
                case "$1" in
                    "boss1") echo "multiagent-akasatanamidakuji:$window_name.$((pane_index))" ;;
                    "worker1") echo "multiagent-akasatanamidakuji:$window_name.$((pane_index + 1))" ;;
                    "worker2") echo "multiagent-akasatanamidakuji:$window_name.$((pane_index + 2))" ;;
                    "worker3") echo "multiagent-akasatanamidakuji:$window_name.$((pane_index + 3))" ;;
                esac
            else
                echo ""
            fi
...
show_agents() {
    ...
    # presidentセッション確認
    if tmux has-session -t "president-akasatanamidakuji" 2>/dev/null; then
...
send_message() {
    ...
    # メッセージ送信（-lでリテラル送信。長文/複数行はペースト扱いになり
    # 描画に時間がかかるため、Enterを送るまでに十分な待機を入れる）
